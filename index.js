...
// Companion must run on the same machine as OBS, or use an SSH tunnel.
//
// API endpoints used:
...
// Required by Companion — migration scripts for settings format changes
...
export default class DskInstance extends InstanceBase {
    constructor(internal) {
        super(internal)
        this.pollTimer  = null
        this.items      = []   // [{ name: string, active: bool, timeRemaining?: number }]
        this.sceneName  = ''
        this.config     = {}
    }
        ...
        return [
            {
                ...
                id:    'info',
                label: 'Connection',
                value: '⚠️ The DSK plugin HTTP server binds to 127.0.0.1 by default, ' +
                       'so Companion must run on the same machine as OBS. ' +
                       'Use an SSH tunnel if Companion is on a separate device.',
                width: 12,
            },
            {
                type:    'textinput',
            ...
            },
            {
                ...
                tooltip: 'How often to check DSK state. 500ms is a good default.',
                default: 500,
                min:     100,
                max:     5000,
                width:   4,
    ...
    startPolling() {
        ...
        this.pollStatus()  // hit it immediately
    ...
    async pollStatus() {
        try {
            ...
            // Detect item list changes → rebuild definitions + presets
            ...
            // Re-evaluate all active/inactive feedbacks
    ...
    updateFeedbacks() {
        ...
        this.setFeedbackDefinitions({
            item_active: {
                ...
                description:  'Button turns green when the DSK item is live (active)',
                defaultStyle: {
    ...
    updateVariableDefinitions() {
        const defs = [
            { variableId: 'scene', name: 'DSK Scene Name' },
        ]
        for (const item of this.items) {
            const k = this.safeKey(item.name)
    ...
    // ── Presets ───────────────────────────────────────────────────────────────
    // One toggle button per DSK item, green when live. Drag onto your layout.
    ...
    // Converts an item name into a safe variable key, e.g. "Sponsor Bug" → "sponsor_bug"
    safeKey(name) {
    ...
    async sendCommand(name, action) {
        ...
        try {
            ...
            if (!res.ok) throw new Error(`HTTP ${res.status}`)
            // Refresh state immediately so feedback updates without waiting for the next poll
